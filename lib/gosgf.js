 ...
 *    {@link GoSgf.GameTree.Node#_raw|Node#_raw} property when parsing SGF data.
 *
 * @constructor
 */
function GoSgf(data, keepRaw) {
  ...
  /**
   * Whether to keep the {@link GoSgf.GameTree.Node#_raw|Node#_raw} property in
...
/**
 * Create a GoSgf object from a JSON representation
 */
GoSgf.fromJSON = function (json, keepRaw) {
  var sgf = new GoSgf(),
    data = typeof json === 'string' ? JSON.parse(json) : json;

  if (data != null) {
    sgf.collection = (data.collection||[]).map(function (gt) {
      return GoSgf.GameTree.fromJSON(gt);
    });
  }
...
GoSgf.prototype = {
  /**
   * Create an object with properties to export in JSON serialization.
   *
   * @return {object} An object with properties to export
   */
  toJSON: function () {
    return {
...
if (typeof window !== 'undefined') window['GoSgf'] = GoSgf;
