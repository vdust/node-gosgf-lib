...
Point.unique = function (list) {
  var i = 0, last;
  list.sort(Point.compare);
  while (i < list.length) {
    if (last && Point.equals(list[i], last)) {
      list.splice(i, 1);
    } else {
      last = list[i++];
    }
...
Point.prototype = {
  ...
  /**
   * Get a hashable representation of the point value
   *
   * Same as .toString() in this case
   *
   * @return {string} The hashable value
   */
  toKey: function () {
    return this.toString();
  },
  /**
   * Check whether a point equals to another.
   *
   * @param {GoSgf.Spec.Point} p - A point-like instance to compare.
   */
  equals: function (p) {
    return this.x === p.x && this.y === p.y;
  },
  compare: function (p) {
    return Point.compare(this, p);
  }
};
...
Composed.prototype = {
  ...
  /**
   * Return The encoded value of first component of the composed value.
   *
   * This is used essentially with Point:* composed instances to get a unique
   *
   */
  toKey: function () {
    return _anyToString(this.first, true);
...
Spec.prototype = {
  ...
  decodeBoardSize: function (key, values) {
    var l, a, b, dsz = this.sizeDefault.slice();
    this._checkRoot(key);
  ...
  /**
   * Map of information name associated to property names.
   * @type {object}
   */
  INFO_MAP: {
    annotator: { p: 'AN' },
    ...
    nodecomment: { p: 'C', node: true },
    blacktimeleft: { p: 'BL', node: true },
    blackmovesleft: { p: 'OB', node: true },
    whitemovesleft: { p: 'OW', node: true },
    whitetimeleft: { p: 'WL', node: true }
  },
  ...
  /**
   * List of node info properties.
   * @type {string[]}
   */
  NODE_INFO: [ 'C', 'BL', 'OB', 'OW', 'WL' ],
