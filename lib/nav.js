...
Nav.prototype = {
  ...
  /**
   * Check if the current cursor is a descendant of a path.
   *
   * @param {GoSgf.Nav|GoSgf.Nav~Path} path - The path to test.
   * @param {boolean} [orSelf] - Whether to consider the current path as a
   *    descendant of itself.
   *
   * @returns {boolean} `true` if the cursor is a descendant of `path`,
   *    `false` otherwise.
   */
  ...
  _comparePaths: function (path1, path2, _dir) {
    ...
    _dir = _dir || 1;
    return (l1 > l2 || path1[i] > path2[i]) ? _dir : -_dir;
  },
  ...
  stones: function (color, points, toggle) {
    ...
    var _needsCleanup = false;
    function test (pRef, k) {
      // Removing a point from 'AE' requires state cleanup (see below)
      _needsCleanup = _needsCleanup || (pRef && pRef.k === 'AE');
    ...
    for (i = 0; i < points.length; i++) {
      ...
      t = (typeof toggle === 'boolean') ? toggle : (itn.color === Intersection.NONE);
      if (t) {
        if (itn.color !== color) {
          mapPush(pointsMap, c, node, p, test);
          // changing intersection color requires state cleanup (see below)
          if (itn.color) _needsCleanup = true;
        }
    ...
    // Need to cleanup stones set to a state that matches that of previous
    // node. Cases includes intersection color change and stone previously in
    // 'AE' property.
    if (_needsCleanup && !self.isRoot() && (node.AB || node.AW)) {
      self.prev();
      bdata = board.board;
      for (i = 0; i < (node.AB||[]).length; i++) {
        p = node.AB[i];
        itn = bdata[p.x + bdata.width * p.y];
        if (itn.color === Intersection.BLACK) {
          node.AB.splice(i, 1);
          i--;
        }
      }
      for (i = 0; i < (node.AW||[]).length; i++) {
        p = node.AW[i];
        itn = bdata[p.x + bdata.width * p.y];
        if (itn.color === Intersection.WHITE) {
          node.AW.splice(i, 1);
          i--;
        }
      }
