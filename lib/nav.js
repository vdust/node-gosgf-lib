 ...
 *
 * @property {GoSgf.Nav~Path} path - The path of the node in the owning GameTree
 * @property {GoSgf.GameTree.Node} node - The node instance, owned by the
 *    GameTree.
 */
...
Nav.prototype = {
  ...
  cut: function () {
    ...
    // FIXME: Should not be required here.
    // FIXME: nav state should track tree changes in some way to allow full
    // FIXME: implicit rendering.
    this.board.clear();
   ...
   *   respectively.
   */
  toggle: function (color, points, toggle) {
    var self = this,
      board = self.board,
      bdata, newnode,
      c, i, p, itn, t, k, list,
    ...
    ([ 'AB', 'AW', 'AE' ]).forEach(function (k) {
      if (!(k in node)) return;
      for (var j = 0; j < node[k].length; j++) {
        pointsMap[node[k][j].toString()] = { k: k, i: j };
      }
    });

    function push(map, point) {
      var pref = pointsMap[point.toString()];
      if (pref) {
        node[pref.k].splice(pref.i, 1);
        for (var k in pointsMap) {
          if (pointsMap[k].k === pref.k) {
            if (pointsMap[k].i > pref.i) pointsMap[k].i--;
          }
        }
      }
      if (!pref || pref.k === 'AE' || map !== 'AE') {
        if (!(map in node)) node[map] = [];
        pointsMap[point.toString()] = { k: map, i: node[map].length };
        node[map].push(new Spec.Point(point));
      }
    }
    ...
    for (i = 0; i < points.length; i++) {
      ...
      if (t) {
        if (itn.color !== color) push(c, p);
      } else if (itn.color !== Intersection.NONE) {
        push('AE', p);
      }
    }

    if (node.AB && !node.AB.length) delete node.AB;
    if (node.AW && !node.AW.length) delete node.AW;
    if (node.AE && !node.AE.length) delete node.AE;

    if (newnode) self.board.clear();
    self.update();
