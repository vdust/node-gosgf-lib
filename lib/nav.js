...
Nav.prototype = {
  ...
  /**
   * Get the {@link GoSgf.Spec.Node|Node} instance at `path`. If `path` not
   * set or `null`, get the node at the current path.
   *
   * @param {?GoSgf.Nav~Path} path - A node path
   *
   * @returns {?GoSgf.Spec.Node} A node instance, or null if the path is
   *    invalid.
   */
  get: function (path) {
   ...
   *
   * @returns {GoSgf.Spec.Node[]} The array of nodes. If `fromPath` is set and
   *    is not an ascendant of the current path, the returned array is empty.
   */
