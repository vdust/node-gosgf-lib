...
Board.prototype = {
   ...
   */
  queueRender: function (nav) {
    this._queuedRender = nav.copy();
  },
  /**
   * Obtain data for a given intersection.
   *
   * @param {Number} x - The horizontal coordinate of the intersection.
   *        If `y` is undefined, `x` is the combined index (`x + width * y`).
   *
  ...
  clear: function (nav) {
    ...
    if (!infos) {
      ...
      for (var k in infos) {
        if (infos.ownProperty(k)) delete infos[k];
      }
    }
  ...
  renderToNode: function () {
    ...
    var self = this,
      ...
      sz = self._size,
      board = self._board,
      IPROPS = self.INFO_PROPERTIES, def,
      MARKS = self.MARKS,
      vw = [],
    ...
    for (i = 0; i < IPROPS.length; i++) {
      def = IPROPS[i];
      if (def.k in self._infos) node[def.p] = self._infos[def.k];
    }
        ...
        if (mark) {
          for (m = 0; m < MARKS.length; m++) {
            ...
            if (def.mark.dimmed && mark.dimmed) {
              ...
              props.push({ k: def.p, v: new Composed(p, mark.label) });
   ...
   *
   * If any group has no liberty left, stones of that group are removed from
   * the board and the captured stones counter is increased accordingly. The
   * board state is assumed to be valid before m0 is played.
   *
  ...
  _checkCaptured: function (m0) {
    ...
    if (itn0.color === itn0.NONE) return;
   ...
   *
   * @param {GoSgf.Spec.Node} node - The node to render
  ...
  _updateInfos: function (node) {
    var infos = this._infos,
      props = this.INFO_PROPERTIES,
      i, def;

    for (i = 0; i < props.length; i++) {
      def = props[i];
      if (def.clear) delete infos[def.k];
      if (def.p in node) infos[def.k] = node[def.p];
    }
  },
  /**
   ...
   *
   * @param {GoSgf.Spec.Node} ndoe  A Node instance
   */
